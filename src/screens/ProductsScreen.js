...
function ConfirmModal({ visible, theme, title, message, onCancel, onConfirm, confirmLabel = 'Delete', danger = true }) {
  const s = confirmStyles(theme);
  return (
    <Modal visible={visible} transparent animationType="fade">
      <View style={s.overlay}>
        <View style={s.box}>
          <View style={[s.topBar, { backgroundColor: danger ? theme.red : theme.accent }]} />
          ...
          <View style={s.btnRow}>
            ...
            <TouchableOpacity
              style={[s.confirmBtn, { backgroundColor: danger ? theme.red : theme.accent }]}
              onPress={onConfirm}
            >
...
export default function ProductsScreen() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await deleteProduct(deleteTarget.id, token);
      if (res.success) {
  ...
  const renderItem = ({ item }) => (
    <View style={s.card}>
      ...
      <View style={s.cardActions}>
        ...
        <TouchableOpacity style={s.delBtn} onPress={() => setDeleteTarget(item)}>
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerCount}>{filtered.length} / {products.length}</Text>
      </View>
...
const styles = (t) => StyleSheet.create({
  ...
  filterChip:         { flexDirection: 'row', alignItems: 'center', backgroundColor: t.surface2, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 7, borderWidth: 1, borderColor: t.border, gap: 6 },
  ...
  filterCount:        { backgroundColor: t.border, borderRadius: 10, paddingHorizontal: 6, paddingVertical: 1 },
  filterCountActive:  { backgroundColor: 'rgba(255,255,255,0.25)' },
  filterCountText:    { color: t.muted, fontSize: 11, fontWeight: '700' },
